...
internal sealed class ChatBot : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested)
        {
                ...
                JokeType jokeType, string language, CancellationToken token)
            {
                var (joke, bot) = await PrepareJokeAsync(jokeType, language, token);
                await SendJokeAsync(joke, bot, language);
                await Task.Delay(_random.Next(7_500, 15_000), cancellationToken);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Error: {Message}. {Error}", ex.Message, ex);
            }
        }
        ...
        JokeType type, string lang, CancellationToken cancellationToken)
    {
        ...
        if (lang is { Length: > 0 } and not "en-US" and not "en")
        {
            var (translatedJoke, isTranslated) = await _translationService.TranslateAsync(joke, lang);
            if (isTranslated)
            {
                _logger.LogInformation(
                    "Joke Bot, translated to {Lang}: {TranslatedJoke}", lang, translatedJoke);
            }
            else
            {
            ...
            return (translatedJoke ?? joke, bot);
        }
